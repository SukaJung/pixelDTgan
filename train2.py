...
class PixelDTgan():
    ...
    def train(self,training_epoch=1000000,batch_size=128):
        ...
        start_point = 0
#         path = "./model2/model2-140.meta"
#         self.loader = tf.train.import_meta_graph('./model2/model2-140.meta')
#         self.loader.restore(self.sess,tf.train.latest_checkpoint('./model2'))
...
#         print(start_point)
        ...
        for epoch in range(start_point,training_epoch):
            ...
            if epoch%50 == 0:
                ...
                plt.savefig('outputs2/{}.png'.format(epoch), bbox_inches='tight')
                plt.close(fig)
                 
            if epoch%50== 0:
                self.saver.save(self.sess, './model2/model2', global_step=epoch)
